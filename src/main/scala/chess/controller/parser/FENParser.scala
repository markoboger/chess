...
object FENParser {
  /**
   * Parses a FEN (Forsyth-Edwards Notation) string and returns a Board.
   * FEN format: "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
   * 
   * @param fen The FEN string
   * @return A Board if parsing succeeds, or an error message
   */
  ...
  private def parseBoardFromFEN(boardPart: String): Board = {
    ...
    if (ranks.length != 8) {
      throw new IllegalArgumentException(s"FEN board must have 8 ranks, got ${ranks.length}")
    }
  ...
  /**
   * Parses a single rank from FEN notation.
   * Example: "rnbqkbnr" or "8" or "r1bqkb1r"
   */
  private def parseRank(rankStr: String): Vector[Option[Piece]] = {
    ...
    if (squares.length != 8) {
      throw new IllegalArgumentException(s"FEN rank must have 8 squares, got ${squares.length}: $rankStr")
    }
  ...
  /**
   * Parses a single piece character from FEN notation.
   * Uppercase = White, Lowercase = Black
   * p/P = Pawn, n/N = Knight, b/B = Bishop, r/R = Rook, q/Q = Queen, k/K = King
   */
  private def parsePiece(char: Char): Option[Piece] = {
    val color = if (char.isUpper) PieceColor.White else PieceColor.Black
  ...
  /**
   * Converts a Board to FEN notation (board position only).
  ...
  private def pieceToFEN(piece: Piece): Char = {
    ...
    if (piece.color == PieceColor.White) char.toUpper else char
  }
}
