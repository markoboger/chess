...
final class CheckSpec extends AnyWordSpec with Matchers:
  ...
  "findKing" should {
    ...
    "find king on a custom position" in {
      val board = FENParser.parseFEN("8/8/8/8/3K4/8/8/8").get
      board.findKing(Color.White) shouldBe Some(Square("d4"))
    }

    "return None when no king is present" in {
      val board = FENParser.parseFEN("8/8/8/8/8/8/8/8").get
  ...
  "isAttackedBy" should {
    "detect pawn attack from white" in {
      val board = FENParser.parseFEN("8/8/8/8/8/4P3/8/4K3").get
    ...
    "detect pawn attack from black" in {
      val board = FENParser.parseFEN("4k3/8/4p3/8/8/8/8/8").get
    ...
    "detect knight attack" in {
      val board = FENParser.parseFEN("8/8/8/8/3N4/8/8/4K3").get
    ...
    "detect bishop attack" in {
      val board = FENParser.parseFEN("8/8/8/8/3B4/8/8/4K3").get
    ...
    "detect rook attack" in {
      val board = FENParser.parseFEN("8/8/8/8/3R4/8/8/4K3").get
    ...
    "detect queen attack on diagonal" in {
      val board = FENParser.parseFEN("8/8/8/8/3Q4/8/8/4K3").get
    ...
    "not detect attack through blocking piece" in {
      ...
      val board = FENParser.parseFEN("8/8/8/8/P7/8/8/R3K3").get
  ...
  "isInCheck" should {
    ...
    "detect check by rook" in {
      val board = FENParser.parseFEN("4k3/8/8/8/8/8/8/4R3").get
      board.isInCheck(Color.Black) shouldBe true
    }

    "detect check by bishop" in {
      // Bishop on b5 attacks e8 diagonally
      val board = FENParser.parseFEN("4k3/8/8/1B6/8/8/8/4K3").get
      board.isInCheck(Color.Black) shouldBe true
    }

    "detect check by knight" in {
      val board = FENParser.parseFEN("4k3/8/5N2/8/8/8/8/4K3").get
      board.isInCheck(Color.Black) shouldBe true
    }

    "detect check by pawn" in {
      val board = FENParser.parseFEN("4k3/3P4/8/8/8/8/8/4K3").get
      board.isInCheck(Color.Black) shouldBe true
    }

    "detect check by queen" in {
      val board = FENParser.parseFEN("4k3/8/8/8/4Q3/8/8/4K3").get
      board.isInCheck(Color.Black) shouldBe true
    }

    "not detect check when line is blocked" in {
      val board = FENParser.parseFEN("4k3/8/8/4p3/4R3/8/8/4K3").get
      board.isInCheck(Color.Black) shouldBe false
    }

    "return false when no king present" in {
      val board = FENParser.parseFEN("8/8/8/8/4R3/8/8/4K3").get
      board.isInCheck(Color.Black) shouldBe false
    }
  }
  ...
  "isCheckmate" should {
    "detect back-rank mate" in {
      ...
      val board = FENParser.parseFEN("3Q2k1/5ppp/8/8/8/8/8/4K3").get
    ...
    "not report checkmate when check can be blocked" in {
      ...
      val board = FENParser.parseFEN("4k3/8/8/r7/8/8/8/4RK2").get
      board.isInCheck(Color.Black) shouldBe true
      board.isCheckmate(Color.Black) shouldBe false
    }
  }
