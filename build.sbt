...
// Classifier string for explicit `org.openjfx` artifacts (e.g. `linux`, `mac-aarch64`).
// Coursier expands OpenJFX POMs and on Linux ARM64 requests `linux-aarch64` jars (not published for 21.0.x);
// Dockerfiles pass `-Dsbt.coursier=false` so Ivy uses the classifiers below. Optional: `-Dchess.openjfx.classifier=…`.
val javafxClassifier = {
  sys.props.get("chess.openjfx.classifier").map(_.trim).filter(_.nonEmpty).getOrElse {
    ...
    // On Linux ARM64, Maven Central does not publish `linux-aarch64` for OpenJFX 21.0.x; use plain `linux`.
    val arch = (sys.props("os.name").toLowerCase, sys.props("os.arch")) match {
      case (n, "aarch64" | "arm64") if n.contains("linux") => ""
...
lazy val LichessBot = project
  ...
  .dependsOn(Core, App)
  .settings(
    name := "Chess-Lichess-Bot",
    scalaVersion := "3.5.0",
    Compile / mainClass := Some("chess.lichess.LichessBotMain"),
    libraryDependencies ++= Seq(
      "org.typelevel" %% "cats-effect" % "3.5.4",
...
lazy val Chess = project
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      "org.scalatest" %% "scalatest" % "3.2.19" % Test,
      // ScalaFX pulls OpenJFX without a stable classifier; pin artifacts below. Docker builds use
      // Ivy (`-Dsbt.coursier=false`) because Coursier expands OpenJFX POMs to `linux-aarch64` on ARM64 Linux.
