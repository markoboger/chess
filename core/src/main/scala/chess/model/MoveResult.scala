...
/** A monadic type representing the result of a chess move.
  *
  * `MoveResult` is either a successful [[MoveResult.Moved Moved]] containing the new board state and a [[GameEvent]],
  * or a [[MoveResult.Failed Failed]] containing the unchanged board and a [[MoveError]] explaining why the move was
  * rejected.
  *
  * Supports `map`, `flatMap`, and `foreach` for monadic composition, enabling for-comprehensions that short-circuit on
...
sealed trait MoveResult:
  ...
  /** Chains a move-producing function. Short-circuits on failure.
    *
    * On [[MoveResult.Moved Moved]], applies `f` to the board producing the next `MoveResult`. On
    * [[MoveResult.Failed Failed]], propagates the failure unchanged.
    */
  ...
  /** `Some((board, event))` on success, `None` on failure. Use this to avoid unreachable `case _` branches when
    * matching on `board.move(...)`.
    */
  def movedOption: Option[(Board, GameEvent)]

object MoveResult:

  /** A successful move: contains the resulting board and the [[GameEvent]] it produced. */
  final case class Moved(board: Board, gameEvent: GameEvent = GameEvent.Moved) extends MoveResult:
    ...
    def movedOption: Option[(Board, GameEvent)] = Some((board, gameEvent))
  ...
  final case class Failed(board: Board, error: MoveError) extends MoveResult:
    ...
    def movedOption: Option[(Board, GameEvent)] = None
