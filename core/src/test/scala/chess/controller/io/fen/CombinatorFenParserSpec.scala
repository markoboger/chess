...
final class CombinatorFenParserSpec extends AnyWordSpec with Matchers with FullFenParserBehaviors:
  ...
  "CombinatorFenParser" should {
    "parse the initial position" in {
      ...
      val result = CombinatorFenParser.parseFEN(fen)
    ...
    "convert board to FEN notation" in {
      ...
      val fen = CombinatorFenParser.boardToFEN(board)
    ...
    "round-trip FEN: parse and convert back" in {
      ...
      val board = CombinatorFenParser.parseFEN(originalFEN).get
      val resultFEN = CombinatorFenParser.boardToFEN(board)
    ...
    "fail on empty FEN string" in {
      val result = CombinatorFenParser.parseFEN("")
    ...
    "produce the same Board as RegexFenParser for initial position" in {
      ...
      val combinator = CombinatorFenParser.parseFEN(fen).get
  ...
  "CombinatorFenParser as FenIO" should {
    val fileIO: FenIO = CombinatorFenParser
    ...
    "be usable polymorphically as FenIO" in {
      val io: FenIO = CombinatorFenParser
    ...
    "be usable polymorphically as FileIO" in {
      val io: chess.controller.io.FileIO = CombinatorFenParser
      val fen = io.save(Board.initial)
      val board = io.load(fen).get
